...
namespace ns3 {
	...
	const uint32_t PaymentNetwork::SEND_LOCKEDBP_PORT = 2000;
	const uint32_t PaymentNetwork::SEND_SECRET_REQ_PORT = 2100;
	const uint32_t PaymentNetwork::SEND_SECRET_REVEAL_PORT = 2200;
	const uint32_t PaymentNetwork::SEND_BP_PORT = 2300;
	 ...
	 switch (tHeader.Get ())
	 {
	  ...
	  case OFFCHAIN_ROUTING_RREP:
	  {
	   ...
	   else if (resEv == 1)
	   {
		...
		int32_t totalAmout = rrepHeader.getAccRewards();
		m_paymentID++;
		SendLockBP(paymentID, dst, totalAmout);
	   }
	   break;
	  }
...
*/
	...
	int
	PaymentNetwork::SendLockBP(uint32_t paymentID, Ipv4Address dst, uint32_t finalAmout)
	{
	 LockBPHeader lockBPHeader;

	 /**
	  *  lockBPHeader setting
	  */

	 //pending payment_entry
	 PendingPaymentEntry entry;
	 entry.initiator = GetNodeAddress();
	 entry.target = dst;
	 entry.hashTimeLock = m_hashTimeLock;
	 entry.unSignedBalanceProof = m_unSignedBalanceProof;
	 entry.secret = m_secret;
	 m_pending_payment_entry.insert(make_pair(paymentID, entry));

	 Ptr<Packet> packet = Create<Packet> ();
	 packet->AddHeader (LockBPHeader);
	 TypeHeader tHeader (SEND_LOCKED_UNSIGNED_BP);
	 packet->AddHeader (tHeader);
	 // Send to all-hosts broadcast if on /32 addr, subnet-directed otherwise
	 Ipv4Address destination = LookupNextNode(dst);
	 NS_LOG_DEBUG ("Send LockBP with id " << LockBPHeader.GetId () << " to socket");
	 socket->SendTo (packet, 0, InetSocketAddress (destination, SEND_LOCKEDBP_PORT));

	 ScheduleLockBPRetry (dst);
	 return 0;
	}

	int
	PaymentNetwork::RecvLockBP (Ptr<Packet> p, Ipv4Address receiver, Ipv4Address src)
	{
	 if (IsMyOwnAddress (target))
	 {
	  // Secret request
	  SendSecretREQ(paymentID, target, initiator, m_hashTimeLock);
	  return 1;
	 }

	 //m_pending_payment_entry
	 PendingPaymentEntry entry;
	 ...
	 // Balance Proof create
	 m_unSignedBalanceProof = new UnSignedBalanceProof(finalAmout);
	 lockBPHeader.SetUnsignedBalanceProof(m_unSignedBalanceProof);
	...
	int
	SendSecretREQ(uint32_t paymentID, Ipv4Address target, Ipv4Address initiator, Ptr<offchain::HashTimeLock> hashTimeLock)
	{
		/**
 		* 	SecretReq header setting
 		*/
	 Ptr<Packet> packet = Create<Packet> ();
	 packet->AddHeader (secretReqHeader);
	 TypeHeader tHeader (SEND_SECRET_REQ);
	 packet->AddHeader (tHeader);

	 Ipv4Address destination = initiator;
	 NS_LOG_DEBUG ("Send secretReq with id " << secretReqHeader.GetId () << " to socket");
	 socket->SendTo (packet, 0, InetSocketAddress (destination, SEND_SECRET_REQ_PORT));

	 ScheduleSecretREQ (initiator);
	 return 0;
	}

	int
	PaymentNetwork::RecvSecretREQ (Ptr<Packet> p, Ipv4Address receiver, Ipv4Address src)
	{
	 NS_LOG_FUNCTION (this);

	 SecretReqHeader secretReqHeader;
	 p->RemoveHeader (secretReqHeader);

	 uint32_t paymentID = lockBPHeader.GetId ();
	 Ipv4Address initiator = lockBPHeader.GetInitiator ();
	 Ipv4Address target = lockBPHeader.GetTarget();
	 m_hashTimeLock = lockBPHeader.GetHashTimeLock();

	 NS_LOG_LOGIC (receiver << " paymentID " << paymentID
							<< " to target " << target
							<< " initiator" << initiator );
